...
class RagAgentWindow(QMainWindow):
        ...
        try:
            ...
            if paths_to_embed:
                if m_type == "dummy" or self.active_model_obj == "dummy":
                    ...
                    for i in range(0, len(paths_to_embed), embed_batch_size):
    ...
    def process_file_batch(self, file_paths: list):
        ...
        for path in file_paths:
            ...
            elif ext == '.csv' and hasattr(self, 'chk_csv') and self.chk_csv.isChecked():
    ...
    def _embed_and_buffer_pdf_pages(self, source_path: str, page_buffer: list):
        """Decoupled PDF page embedding & buffering workflow."""
        if not self.table:
            if not self.connect_db(): return
        ...
        try:
            ...
            img_paths = [p[1] for p in page_buffer]
            img_hashes = [self._calc_image_hash(p) for p in img_paths]
            ...
            paths_to_embed = [p for p, h in zip(img_paths, img_hashes) if h and h not in hash_to_vec]
            computed_vectors = {}
            ...
            if paths_to_embed:
                if m_type == "dummy" or self.active_model_obj == "dummy":
                    ...
                    with torch.no_grad():
                        self.active_model_obj.eval()
                        images = [Image.open(p).convert("RGB") for p in paths_to_embed]
                        features = self.active_model_obj.encode(images)
                        if hasattr(features, "detach"):
                            features = features.detach().cpu().numpy()
                        features = np.array(features, dtype=np.float32)
            ...
            rows = []
            for text, img_path, meta_img in page_buffer:
                ih = self._calc_image_hash(img_path)
                i_vec = hash_to_vec.get(ih, computed_vectors.get(img_path, [0.0]*dim))
                t_vec = [0.0]*dim 
                ...
                rows.append({
                    ...
                    "image_path": img_path,
                    "metadata": meta_img,
                    "text_hash": self._calc_hash(text),
                    "image_hash": ih,
            ...
            self._accumulate_and_flush("image_hash", rows)
            
        except Exception as e:
            self.log_diag(f"[PDF ERROR] Embedding/buffer failed: {e}")
    ...
    def process_pdf_file(self, path: str):
        ...
        self.log_diag(f"[PDF] Extracting text and images from {os.path.basename(path)}...")
        try:
            doc = fitz.open(path)
            page_buffer = []
            ...
            for i in range(len(doc)):
                ...
                img_path = f"{path}_page_{i+1}.png"
                pix.save(img_path)
                
                meta_img = json.dumps({"source": os.path.basename(path), "page": i+1, "format": "visual_slice"})
                page_buffer.append((text, img_path, meta_img))
                ### diagnostics on terminal
                print(f"PDF page_buffer status: {len(page_buffer)} / {buffer_limit}")
                if len(page_buffer) >= buffer_limit:
                    self._embed_and_buffer_pdf_pages(path, page_buffer)
                    page_buffer.clear()
                    QApplication.processEvents()
    ...
    def _get_base64_image(self, image_path: str, metadata: dict) -> str:
        ...
        # Scenario B: Image is missing, but it was a PDF slice. Handle the exception via PyMuPDF.
                ...
                if os.path.exists(original_pdf_path):
                    try:
                        ...
                        if 0 <= page_idx < len(doc):
                            ...
                            self.log_diag(f"[B64 RECOVERY] Live rendered missing page {page_num} from {os.path.basename(original_pdf_path)}")
