...
A general-purpose Multimodal Retrieval-Augmented Generation (RAG) server built for storing and searching artistic references. It ingests images, PDFs, compressed image archives, videos, and text, and exposes a ZeroMQ TCP API so any front-end application can perform semantic, full-text, and cross-modal searches.
...
*Note: I created this as a tool to help artists quickly search for drawing references through large libraries. At some point, looking through tens of thousands of images or videos to locate the exact reference you might remember was there becomes too time-consuming and an actual chore. I felt the need to automate all of that and I wanted something very specific, and scalable to millions of records.*

My idea is to help with the search process, offering a much smaller subset of candidate references than can be searched by image (even by importing a quick hand-drawn sketch through a webcam) or by natural language, or by booru-style tags. Images can also be imported with their own companion metadata files with additional textual descriptions of the contents.
...
```bash
python rag_server_gui12z4_clip-vit_fast.py
```
...
```bash
python rag_agent_frontend4.py
```

### 🧠 Available CLIP-ViT Models
You can download and switch between models directly in the server GUI. The models are autodownloaded from Huggingface.
...
| **CLIP ViT-L-14** | 768 | Recommended for maximum accuracy. *(Note: Requires a new table due to 768 dim)*. |
...
### 🖥️ Companion Frontend Application
A fully functional GUI (`rag_agent_frontend4.py`) is included as both a proof-of-concept and a practical tool for artists. It allows you to directly interact with the server, perform multimodal searches, and organize your artistic references.

**Key Highlights:**
* **Search & Capture:** Perform text, image, or multimodal searches. Capture live references directly from UVC webcams or ESP32-CAM modules.
* **Search Results Viewer:** The essential hub for browsing, diagnosing, and **exporting** search results. Use it to visually inspect matches, view metadata/payloads, and save selected images directly to your local folders for mood boards.

**⚠️ Prerequisites for a Functional Workflow:**
To perform searches from the frontend, you must ensure:
1. The RAG server is running and the **Start TCP Server** button is enabled.
2. Your database tables are populated with references and actively loaded.
3. The **IP Address** and **Port** in the frontend's *Configuration* tab match the server's settings (Default: `127.0.0.1:5001`).

*(The interface is designed to be intuitive. Simply load your DB on the server, start the TCP listener, match the IP/Port in the frontend, and start searching!)*
